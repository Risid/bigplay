	...
	struct mad_header const *header, char const *label, int now)
{
	...
	if (seconds != stats->nsecs || !on_same_line || now) {
		...
		if (label || stats->label) {
			message("播放[%s] %s", time_str, label ? label : stats->label);
			...
			message(_("播放[%s] 音量减少[k] 音量增加[i] 暂停[p] 上一首[j] 下一首[l] 退出[q]"),
				time_str
				);
	...
	while (count && (player->repeat == -1 || player->repeat--)) {
		while (playlist->current < playlist->length) {
			...
			case PLAYER_CONTROL_NEXT:
				if (playlist->length == playlist->current + 1)
				{
					playlist->current = 0;
					message("%s", "从第1首歌开始播放");
				}
...
again:
	...
	case KEY_PAUSE:
		...
		message(" --%s--", stopped ? _("已停止 按p重新播放") : _("已暂停 按p继续播放"));
...
int player_run(struct player *player)
{
	...
	FILE *fp;
	if ((fp = fopen(player->file, "r")) == NULL)
	{
		return -1;
	}
	int i = 0;
	while (!feof(fp))
	{
		fgets(temp[i], 1000, fp);
		i++;
	}
	char **play_list = malloc(sizeof(char) * 100 * i);
	message("歌曲列表\n");
	for (int j = 0; j < i; ++j)
	{
		play_list[j] = strtok(temp[j], "\n");
		message("%d. %s\n", j+1, play_list[j]);
	}


	player->playlist.entries = play_list;
	player->playlist.length = i;
